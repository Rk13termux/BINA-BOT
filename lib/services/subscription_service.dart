...
class SubscriptionService extends ChangeNotifier {
  ...
  bool get isPremium =>
      _subscriptionTier == premiumTier || _subscriptionTier == proTier;
  ...
  Future<void> _loadSubscriptionData() async {
    try {
      ...
      _logger.info(
          'Loaded subscription data: tier=$_subscriptionTier, subscribed=$_isSubscribed');
    } catch (e) {
  ...
  Future<void> startTrialSubscription({String tier = premiumTier}) async {
    try {
      ...
      _subscriptionExpiry =
          DateTime.now().add(Duration(days: 7)); // 7-day trial
