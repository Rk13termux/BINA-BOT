...
class BinanceWebSocketService extends ChangeNotifier {
  ...
  Future<void> subscribeToTicker(String symbol) async {
    if (!_isConnected) {
      _logger
          .warning('Not connected to WebSocket. Cannot subscribe to $symbol');
      return;
    }
  ...
  Future<void> subscribeToMultipleTickers(List<String> symbols) async {
    if (!_isConnected) {
      _logger
          .warning('Not connected to WebSocket. Cannot subscribe to symbols');
      return;
    }
  ...
  void _handleMessage(dynamic message) {
    try {
      ...
      if (data['e'] == '24hrTicker') {
        ...
        _logger.debug(
            'Updated $symbol: \$${price.toStringAsFixed(2)} (${change.toStringAsFixed(2)}%)');
      }
  ...
  void _scheduleReconnection() {
    ...
    _logger.info(
        'Scheduling reconnection in ${delay.inSeconds} seconds (attempt ${_reconnectAttempts + 1}/$_maxReconnectAttempts)');
    ...
    try {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        if (_prices.containsKey('BTCUSDT')) {
          _logger.info(
              'Connection test successful. BTC price: \$${getFormattedPrice('BTCUSDT')}');
          return true;
        }
      }
